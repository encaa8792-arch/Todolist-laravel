...
@section('content')
            ...
            <div class="report-card">
                ...
                @if($topCategory)
                    <div class="report-card-stat">
                        <span class="report-card-stat-label">{{ $topCategory }}</span>
                        <span class="report-card-stat-value pink">{{ $categoryStats[$topCategory] }} tugas</span>
                    </div>
                @endif
                <div style="margin-top: 12px;">
                    @foreach($categoryStats as $cat => $count)
                        <div class="mini-stat">
                            ...
                            <span class="mini-stat-bar" style="width: {{ max(($count / max($categoryStats)) * 100, 10) }}%"></span>
                        ...
                        @foreach($overdueTaskList as $task)
                            <div class="overdue-item">
                                <div class="overdue-info">
                                    <span class="overdue-name">{{ $task->task }}</span>
                                    @if($task->category)
                                        <span class="overdue-category">{{ $task->category }}</span>
                                    @endif
                                </div>
                                <span class="overdue-date">{{\Carbon\Carbon::parse($task->deadline)->format('d M')}}</span>
                            </div>
                        @endforeach
                    </div>
                @else
    ...
    <script>
        ...
        document.getElementById('piePeriodFilter').addEventListener('change', function() {
            ...
            @if(isset($categoryStatsByPeriod) && $categoryStatsByPeriod)
                const allPeriodData = @json($categoryStatsByPeriod);
                if (period === 'all') {
        ...
        document.getElementById('barPeriodFilter').addEventListener('change', function() {
            ...
            @if(isset($weeklyStatsByPeriod) && $weeklyStatsByPeriod)
                const allWeeklyData = @json($weeklyStatsByPeriod);
                if (period === 'week') {
