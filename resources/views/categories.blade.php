...
@section('content')
    ...
    <div class="category-list" id="categoryList">
        @forelse($allCategories as $cat)
            <div class="category-item" data-name="{{ strtolower($cat['name']) }}" data-id="{{ $cat['id'] ?? '' }}" data-default="{{ $cat['is_default'] ?? false }}">
                ...
                <div class="category-actions">
                    <button class="btn-edit" data-tooltip="Edit" onclick="openEditModal('{{ $cat['id'] ?? '' }}', '{{ $cat['name'] ?? '' }}', '{{ $cat['icon'] ?? '📁' }}')">✏️</button>
                    <button class="btn-delete" data-tooltip="Hapus" onclick="deleteCategory('{{ $cat['id'] ?? '' }}', this)">🗑️</button>
                </div>
            </div>
        @empty
